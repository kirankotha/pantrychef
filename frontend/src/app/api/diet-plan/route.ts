...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error?.status === 429 || error?.code === 'rate_limit_exceeded') {
      ...
      const waitMatch = msg.match(/try again in (\d+m[\d.]+s|[\d.]+s)/)
      const wait = waitMatch ? ` Try again in ${waitMatch[1]}.` : ' Please wait a moment and try again.'
      return NextResponse.json({ error: `AI rate limit reached.${wait}` }, { status: 429 })
    }
